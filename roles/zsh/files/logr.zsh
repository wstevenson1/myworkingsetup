#!/usr/bin/env zsh
# zsh port of ~/.logr.bash -- logging utility wrapping the system logger(1).
# Only difference from the bash version: the call-stack lookups use zsh's
# $funcstack instead of bash's $FUNCNAME. Sourced from ~/.zshrc; scripts can
# also `source ~/.logr.zsh` directly.
#
# Usage:
#   logr start [LOG_NAME]          # begin logging (LOG_NAME defaults to "scripts")
#   logr [log|notice|info|debug|warn|error] MESSAGE
#   logr MESSAGE                   # defaults to "user.info" facility
